...
# === ИНСТРУКЦИИ ДЛЯ УМНОГО РАЗМЕЩЕНИЯ ===
SMART_PRICING_HELP = """
## 🧠 Руководство по умному размещению ордеров

### 📊 **Макс. спред (%)**
**Диапазон:** 0.1% - 5.0% | **Рекомендуемое:** 0.5%

- **Что это:** Максимально допустимая разница между лучшей ценой покупки и продажи
- **Низкие значения (0.1-0.3%):** Исполнение только при очень узких спредах (идеально для высоколиквидных пар)
- **Средние значения (0.3-0.8%):** Баланс между качеством исполнения и частотой сделок
- **Высокие значения (0.8%+):** Позволяет торговать менее ликвидными парами, но с большими затратами

⚠️ **Внимание:** При широком спреде торговля продолжается, но с предупреждением

---

### ⚖️ **Порог дисбаланса объемов**
**Диапазон:** 1.5 - 5.0 | **Рекомендуемое:** 2.0

- **Что это:** При каком соотношении объемов bid/ask корректировать размер позиции
- **Низкие значения (1.5-2.0):** Чувствительная реакция на изменения спроса/предложения
- **Средние значения (2.0-3.0):** Умеренная корректировка при значительном дисбалансе
- **Высокие значения (3.0+):** Корректировка только при критическом дисбалансе

🔍 **Принцип работы:**
- Если объем покупок в 2 раза больше продаж → давление покупателей
- Если объем продаж в 2 раза больше покупок → давление продавцов
- При давлении против нашей позиции → уменьшаем размер

---

### 📉 **Коэф. уменьшения позиции**
**Диапазон:** 0.5 - 0.95 | **Рекомендуемое:** 0.8

- **Что это:** На сколько уменьшать размер позиции при неблагоприятных условиях
- **Агрессивные значения (0.5-0.7):** Сильное уменьшение позиции для защиты
- **Умеренные значения (0.7-0.85):** Балансированный подход
- **Консервативные значения (0.85-0.95):** Минимальная корректировка

💡 **Пример:** При коэффициенте 0.8 позиция $100 уменьшится до $80 при неблагоприятных условиях

---

### 📚 **Глубина стакана**
**Диапазон:** 5 - 20 уровней | **Рекомендуемое:** 10

- **Что это:** Сколько уровней ордеров анализировать в стакане
- **Малая глубина (5-8):** Быстрый анализ поверхностной ликвидности
- **Средняя глубина (8-12):** Оптимальный баланс точности и скорости
- **Большая глубина (12-20):** Детальный анализ, но медленнее

⚡ **Производительность:** Больше уровней = точнее анализ, но медленнее исполнение

---

## 📈 **Рекомендуемые настройки по стилям торговли**

### 🏃 **Скальпинг (быстрые сделки)**
- Макс. спред: **0.2%**
- Порог дисбаланса: **1.8**
- Коэф. уменьшения: **0.7**
- Глубина стакана: **8**

### 📊 **Дневная торговля**
- Макс. спред: **0.5%**
- Порог дисбаланса: **2.0**
- Коэф. уменьшения: **0.8**
- Глубина стакана: **10**

### 🎯 **Свинг торговля**
- Макс. спред: **1.0%**
- Порог дисбаланса: **2.5**
- Коэф. уменьшения: **0.85**
- Глубина стакана: **15**
...
1. **Анализ стакана** → Получение данных о ликвидности
2. **Оценка спреда** → Проверка условий для исполнения
3. **Анализ объемов** → Определение давления покупателей/продавцов
4. **Корректировка размера** → Адаптация позиции под условия
5. **Исполнение ордера** → Размещение с оптимальными параметрами

## 📊 **Метрики в логах**
После исполнения ищите в логах:
📊 АНАЛИЗ СТАКАНА BTCUSDT:
💰 Лучшие цены: bid=43250.50, ask=43251.00
📏 Спред: 0.12% (good)
📈 Объемы: bid=125.50, ask=89.20
⚖️ Дисбаланс: +17.0% (buy_pressure)
🔧 Корректировка: 0.1000 → 0.1000 (0.0%)

## ⚠️ **Важные моменты**

- Умное размещение НЕ гарантирует лучшее исполнение в 100% случаев
- При ошибках анализа система автоматически переключается на обычное исполнение
- Все данные анализа сохраняются для последующей оптимизации настроек
- Функция работает только для рыночных ордеров (Market Orders)
"""
...
with tabs[6]:
  ...
  with st.form("settings_form"):
    ...
    with col1:
      ...
      st.divider()
      st.subheader("🧠 Умное размещение ордеров")

      # Используем expander вместо кнопки для инструкции
      with st.expander("📖 Подробная инструкция по умному размещению", expanded=False):
        st.markdown(SMART_PRICING_HELP)

      # Основные настройки умного размещения
      use_smart_pricing = st.checkbox(
        "Включить умное размещение",
        value=trade_cfg.get('use_smart_pricing', True),
        help="Анализ стакана ордеров для оптимизации исполнения"
      )

      if use_smart_pricing:
        st.info("💡 **Совет:** Наведите курсор на названия параметров для быстрых подсказок")
        ...
        with col_smart1:
          max_spread = st.slider(
            ...
            min_value=0.1,
            max_value=5.0,
            value=float(trade_cfg.get('smart_pricing', {}).get('max_spread_pct', 0.5)),
            step=0.1,
            help="""
            🎯 Максимально допустимый спред для исполнения

            • 0.1-0.3%: Только узкие спреды (высоколиквидные пары)
            • 0.3-0.8%: Баланс качества и частоты (рекомендуемо)  
            • 0.8%+: Позволяет менее ликвидные пары

            ⚠️ При превышении торговля продолжается с предупреждением
            """
          )
          ...
          volume_threshold = st.slider(
            ...
            min_value=1.5,
            max_value=5.0,
            value=float(trade_cfg.get('smart_pricing', {}).get('volume_imbalance_threshold', 2.0)),
            step=0.1,
            help="""
            🔍 При каком соотношении bid/ask корректировать размер

            • 1.5-2.0: Чувствительная реакция на дисбаланс
            • 2.0-3.0: Умеренная корректировка (рекомендуемо)
            • 3.0+: Только при критическом дисбалансе

            📊 Пример: если покупок в 2 раза больше продаж → давление покупателей
            """
          )
        ...
        with col_smart2:
          reduction_factor = st.slider(
            ...
            min_value=0.5,
            max_value=0.95,
            value=float(trade_cfg.get('smart_pricing', {}).get('position_reduction_factor', 0.8)),
            step=0.05,
            help="""
            💰 Насколько уменьшать позицию при неблагоприятных условиях

            • 0.5-0.7: Агрессивное уменьшение для защиты
            • 0.7-0.85: Балансированный подход (рекомендуемо)
            • 0.85-0.95: Минимальная корректировка

            💡 При 0.8: позиция $100 станет $80 при плохих условиях
            """
          )

          order_book_depth = st.slider(
            "📚 Глубина стакана",
            min_value=5,
            max_value=20,
            value=int(trade_cfg.get('smart_pricing', {}).get('order_book_depth', 10)),
            help="""
            📊 Количество уровней стакана для анализа

            • 5-8: Быстрый анализ поверхностной ликвидности
            • 8-12: Оптимальный баланс точности и скорости
            • 12-20: Детальный анализ (медленнее)

            ⚡ Больше уровней = точнее, но медленнее
            """
          )

        # === БЛОК ТЕКУЩЕГО СТАТУСА ===
        st.write("📊 **Текущие настройки умного размещения:**")

        metric_col1, metric_col2, metric_col3, metric_col4 = st.columns(4)

        with metric_col1:
          st.metric("Макс. спред", f"{max_spread}%",
                    delta="Узкий" if max_spread <= 0.3 else "Широкий" if max_spread >= 0.8 else "Средний")

        with metric_col2:
          st.metric("Порог дисбаланса", f"{volume_threshold}",
                    delta="Чувствительный" if volume_threshold <= 2.0 else "Консервативный")

        with metric_col3:
          protection_level = "Высокая" if reduction_factor <= 0.7 else "Низкая" if reduction_factor >= 0.9 else "Средняя"
          st.metric("Защита", f"{(1 - reduction_factor) * 100:.0f}%", delta=protection_level)

        with metric_col4:
          analysis_speed = "Быстрый" if order_book_depth <= 8 else "Медленный" if order_book_depth >= 15 else "Средний"
          st.metric("Глубина", f"{order_book_depth} ур.", delta=analysis_speed)

      else:
        # Показываем значения по умолчанию когда функция выключена
        st.info("🔒 Умное размещение выключено. Будет использоваться стандартное исполнение ордеров.")
        max_spread = 0.5
    ...
    if submitted:
      ...
      new_config['trade_settings'] = {
        ...
        "roi_stop_loss_pct": roi_sl,
        "roi_take_profit_pct": roi_tp,
        "use_smart_pricing": use_smart_pricing,
        "smart_pricing": {
          "max_spread_pct": max_spread,
          "volume_imbalance_threshold": volume_threshold,
